...
def build_css():
    ...
    .stApp {
        background:
            radial-gradient(circle at 8% 0%, rgba(99, 102, 241, 0.12), transparent 28%),
            radial-gradient(circle at 92% 8%, rgba(20, 184, 166, 0.10), transparent 24%),
            #f6f8fc;
    }
    ...
    .timer-shell {
        background: linear-gradient(145deg, #111827 0%, #172554 55%, #0f172a 100%);
        border-radius: 30px;
        min-height: 640px;
        padding: 28px;
        position: relative;
    ...
    .mode-dot {
        ...
        border-radius: 50%;
        background: #818cf8;
        box-shadow: 0 0 0 5px rgba(129,140,248,0.10);
    }
...
def render_header():
    st.markdown(
        ...
        <div class="timer-header">
            <div class="eyebrow">◉ Study Focus System</div>
...
def render_metrics(today_sessions):
    ...
    with c2:
        st.markdown(
            f"""
            <div class="metric-card">
                <div class="metric-title">Sessions done</div>
...
def render_timer_engine():
    ...
    with st.container():
        st.markdown(
            f"<div style='background:linear-gradient(145deg,#111827,#172554);padding:24px;border-radius:24px;max-width:820px;margin:0 auto;'>"
            f"<div style='display:flex;justify-content:space-between;align-items:center;color:white;margin-bottom:20px;'>"
            f"<span style='background:rgba(255,255,255,0.1);padding:8px 12px;border-radius:999px;font-size:12px;font-weight:700;'>{mode_text}</span>"
            ...
            f"background:conic-gradient(#818cf8 {get_progress_angle()}deg, rgba(255,255,255,0.10) {get_progress_angle()}deg);"
            f"box-shadow:0 18px 70px rgba(0,0,0,0.18);position:relative;border:1px solid rgba(255,255,255,0.06);'>"
            f"<div style='position:absolute;inset:12px;border-radius:50%;background:#111827;border:1px solid rgba(255,255,255,0.08);display:flex;align-items:center;justify-content:center;flex-direction:column;'>"
            f"<div style='font-size:12px;color:#b8c2d5;font-weight:700;margin-bottom:8px;'>{st.session_state.task_name}</div>"
    ...
    with action1:
        if st.button(action_label, use_container_width=True, key="main_timer_action"):
            if st.session_state.timer_running:
                sync_remaining_time()
                st.session_state.timer_running = False
                st.session_state.end_time = None
    ...
    with action2:
        if st.button("Reset", use_container_width=True, key="reset_timer"):
            reset_timer()
            st.rerun()
...
def render_timer_view():
    ...
    with right:
        ...
        st.markdown(
            """
            <div class="metric-card">
                <div class="metric-title">Current settings</div>
                <div style="color:#101828;font-size:1rem;font-weight:850;">
                    Focus &nbsp;·&nbsp; Break
                </div>
                <div style="margin-top:8px;color:#667085;font-size:0.86rem;">
                    %d min &nbsp; / &nbsp; %d min
                </div>
            </div>
            """
...
if __name__ == "__main__":
    # If this file is run by itself, set its page metadata.
    # When imported into your existing EduSearch AI app, call
    # render_timer_view() from the parent app instead.
    try:
        st.set_page_config(
